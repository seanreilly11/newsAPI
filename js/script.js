		...
		$.ajax({
			...
			success: function(data){
				...
				for(var i = 0; i < data.articles.length; i++){
					...
					document.getElementById("output").innerHTML += '<div class="card col-4"><img src="'+data.articles[i].urlToImage+'" class="card-img-top">'
					+ '<div class="card-body"><h5 class="card-title">' + data.articles[i].title +'</h5>'
					+ '<p class="card-text">' + data.articles[i].description + '</p>'
					+ '<a class="card-text" href="' + data.articles[i].url +'">View story here</a></p>'
					+ '</div></div>';
				}
			},
			error: function(){
		...
		$.ajax({
			...
			success: function(data){
				...
				for(var i = 0; i < data.articles.length; i++){
					...
					+ '<div class="card-body"><h5 class="card-title">' + data.articles[i].title +'</h5>'
					+ '<p class="card-text">' + data.articles[i].description + '</p>'
					+ '<a class="card-text" target="_blank" href="' + data.articles[i].url +'">View story here</a></p>'
					+ '</div></div>';
				}
			},
			error: function(){
